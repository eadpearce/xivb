...
class Character extends Component {
  ...
  render() {
    ...
    if (this.state.loaded) {
      ...
      if (character.data.data.grand_company) grand_company = <p className="cb grd-silver play f4"><img height="32" className="v-mid" src={character.data.data.grand_company.icon}/> {character.data.data.grand_company.name} {character.data.data.grand_company.rank}</p>;
    }
    ...
    (
      <main className={classLists.container}>
        ...
        {classjobs.map((job, id) => {
          return <div className="fl ph1" key={job.id}><img src={job.data.icon} height="32" className="v-mid"/>
        ...
        <p><img height="32" className="v-mid" src={character.data.data.active_class.role.icon}/> {character.data.data.active_class.role.name}</p>
        </div>
        ...
        {crafterClasses.map((crafter, id) => {
          return <div className="fl ph1" key={crafter.id}><img src={crafter.data.icon} height="32" className="v-mid"/>
